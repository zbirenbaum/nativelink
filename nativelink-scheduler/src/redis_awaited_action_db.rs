...
impl RedisAwaitedActionDb {
    pub async fn new(
        ...
        let subscriber = store.get_subscriber_client();
    ...
    async fn add_action(
        ...
        match self.get_operation_id_by_hash_key(&action_info.unique_qualifier).await {
            Ok(Some(operation_id)) => {
                ...
                let _: String = self.client.set(key.into(), operation_id.into(), None, None, false).await?;
            ...
            Ok(None) => {
                ...
                self.client.set(operation_id_key.into(), serde_json::to_string(&action).unwrap(), None, None, false).await?;
