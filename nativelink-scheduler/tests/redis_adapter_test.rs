...
use nativelink_worker::utils::local_worker_test_utils::{
...
mod redis_adapter_test {
    ...
    #[tokio::test]
    async fn test_platform_properties() -> Result<(), Error> {
        const SALT: u64 = 1000;
        let action_digest = DigestInfo::new([3u8; 32], 10);
        let known_properties: HashMap<String, PropertyType> = [
            ("foo0".to_string(), PropertyType::minimum),
            ("bar0".to_string(), PropertyType::minimum),
            ("foo1".to_string(), PropertyType::exact),
            ("bar1".to_string(), PropertyType::exact),
        ]
        .into_iter()
        .collect();
        let action_properties: HashMap<String, PlatformPropertyValue> = [
            ("foo0".to_string(), PlatformPropertyValue::Minimum(0)),
            ("bar0".to_string(), PlatformPropertyValue::Minimum(1)),
            (
                "foo1".to_string(),
                PlatformPropertyValue::Exact("foo".to_string()),
            ),
            (
                "bar1".to_string(),
                PlatformPropertyValue::Exact("bar".to_string()),
            ),
        ]
        .into_iter()
        .collect();
        let worker_properties: HashMap<String, PlatformPropertyValue> = [
            ("foo0".to_string(), PlatformPropertyValue::Minimum(0)),
            ("bar0".to_string(), PlatformPropertyValue::Minimum(1)),
            (
                "foo1".to_string(),
        ...
        let action_info = ActionInfo {
            ...
            platform_properties: PlatformProperties::new(action_properties),
            priority: 0,
        ...
        let adapter = RedisAdapter::new("redis://localhost".to_string(), known_properties);
        adapter.create_new_action(&action_info);
        Ok(())
    }
    ...
    //     let adapter = RedisAdapter::new(client, p);
